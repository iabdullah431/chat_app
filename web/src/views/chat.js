...
class Chat extends React.Component {
    ...
    initSocketConnection = () => {
        let socket = socketIO('ws://localhost:3001',{
        ...
        socket.on('users_status',this.updateUserState);
      ...
      updateUserState = users => {
        let contacts = this.state.contacts;
        contacts.forEach((element, index)=>{
          if(users[element.id]) contacts[index].state = users[element.id];
        });
        this.setState({contacts});
	...
	render() {
		...
		return (
           ...
           typing={this.state.typing} toggle={this.userProfileToggle} />
					{this.renderChat()}
